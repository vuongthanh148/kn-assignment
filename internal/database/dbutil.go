...
import (
	...
	"github.com/kwanpham2195/go-gcp-boilerplate/internal/config/serverconfig"
	"github.com/kwanpham2195/go-gcp-boilerplate/internal/derrors"
	"github.com/kwanpham2195/go-gcp-boilerplate/internal/testing/testhelper"
)
...
func DBConnURI(dbName string) string {
	var (
		...
		password = serverconfig.GetEnv("GO_DISCOVERY_DATABASE_PASSWORD", "postgres")
...
func ConnectAndExecute(uri string, dbFunc func(*sql.DB) error) (outerErr error) {
	...
	if err != nil {
		return fmt.Errorf("%w: %v", derrors.ErrNotFound, err)
	}
	defer func() {
